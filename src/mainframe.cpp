...
void MainFrame::OnMenuTorrentOpenDir( wxCommandEvent& event )
{
	...
	if( pTorrent )
	{
		...
		wxExecute( _T( "Explorer.exe /select, \"" ) + pTorrent->config->GetDownloadPath() + wxConvUTF8.cMB2WC(pTorrent->info->name().c_str() ) + wxT("\""), wxEXEC_ASYNC, nullptr );
...
void MainFrame::OnAbout( wxCommandEvent& event )
{
	...
	info.SetCopyright( _T( "(c) 2019 Bitswash Developers" ) );
	...
	if( m_btsession && m_btsession->GetTorrentQueueSize() > 0 )
	{
		...
		if( total > 0 )
		{
			for( size_t i = 0; i < total; ++i )
			{
				...
				if(torrent && torrent->isvalid &&
					(( torrent_state == TORRENT_STATE_STOP) || ( torrent_state == TORRENT_STATE_PAUSE)))
				{
					enable = true;
					break;
				}
